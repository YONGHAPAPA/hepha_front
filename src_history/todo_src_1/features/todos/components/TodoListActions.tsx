...
class TodoActions extends React.Component<Props, State> {
    render() {
        ...
        return(
            <section>
                <button type="button" onClick={()=> loadTodos(undefined, undefined)} disabled={isLoading} >load</button>
                <button type="button" onClick={()=> saveTodos(undefined, undefined)} disabled={isLoading} >save</button>
            </section>
        );
    }
}
